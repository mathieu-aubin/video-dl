...
# Get the json
tmpjson="$(wget.exe http://www.rai.it/dl/portale/html/palinsesti/replaytv/static/"$ch"_$day.html -qO-)"
...
[ "$url" != "" ] && echo "$url" | grep -q 'creativemedia\|wms' && base="$url" || base=$(curl.exe -w "%{url_effective}\n" -L -s -I -S "$dl" -A "" -o /dev/null); checkurl
 }
...
URLS="$(wget.exe -q -O - $(echo "$page" | sed '/starter/!d;s/.*src\=\"//;s/\".*//') | sed '/src:.*\|src_.*/!d;s/.*\: \"//;s/\".*//')"
...
URLS="$(wget.exe "http://cdnselector.xuniplay.fdnames.com/GetCDN.aspx?streamid=$id" -O - -q -U="" | sed 's/</\
